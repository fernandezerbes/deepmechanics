...
class MechanicalModel(Model):
    ...
    def __init__(self, approximator, grid, dirichlet_bcs, functional, kinematic_law, material_model):
    ...
    def solve(self, epochs=100, early_stopping=True, optimizer=None, **kwargs):
        ...
        for i in range(epochs + 1):
            ...
            # Get energy values
            internal_energy = self.functional.internal_term(ex, ey, gamma_xy, nx, ny, nxy, weights, jacobian_dets)

            external_energy = self.functional.external_term(self.get_displacements)

            source_energy = self.functional.source_term(ux, uy, coords, weights, jacobian_dets)

            print("\033[1mEpoch\033[0m = {}\t \033[1mInternal energy\033[0m = {:.4e}\t \033[1mExternal energy\033[0m = {:.4e}\t \033[1mSource energy\033[0m = {:.4e}".format(
                    i, internal_energy, external_energy.detach().numpy(), source_energy.detach().numpy()))
