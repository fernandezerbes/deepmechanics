from torch import device, ones, tensor, float64
...
def get_derivative(y, x, n, device=device("cpu")):
    if n == 0:
        ...
        dy_dx = grad(y, x, ones(x.size()[0], 1, device=device), create_graph=True, retain_graph=True)[0]
...
def plot_grid(grid):
    ...
    for cell in grid.leaf_cells:
        if cell.is_active:
            rect = Rectangle((cell.x_start, cell.y_start), cell.length_x, cell.length_y, ec="r", fc="k")
